...
export class Email{
	async welcomeEmail (email) {
                ...
                text: 'Hi ' + data['name'] + ' and welcome to Chitchat!\n\nWe are very excited to have you as a new member! Chitchat is designed by college students, for college students to give people a way to make new friends around the country.\n\nWe can\'t wait to see you out there! Go over to chitchat.buzz to get chatting.\n\n\The Chitchat Team'
                };
                if(process.env.SENDGRID_API_KEY) {
                        sgMail.send(msg);
                } else {
                        console.log("Development environment detected. We would be sending: "+JSON.stringify(msg))
                }
	}
